...
export default function SaarthiHomeChat({ profile, onProfileUpdate, setActiveTab, language = 'en' }) {
  ...
  const getInitialGreeting = (lang) => {
    if (lang === 'hi') {
      return "Namaste! Main UdyamSarthi hoon. Main aapko aapke liye suitable business choose aur plan karne mein help karungi.\n\nSabse pehle, aapka naam kya hai?";
    }
    if (lang === 'te') {
      return "నమస్కారం! నేను ఉద్యమ్‌సారథిని. మీకు తగిన వ్యాపారాన్ని ఎంచుకోవడంలో మరియు ప్రణాళిక రూపొందించడంలో నేను సహాయం చేస్తాను.\n\nముందుగా, మీ పేరు ఏమిటి?";
    }
    return "Hello! I am UdyamSarthi. I will help you choose and plan a suitable business for you.\n\nFirst, what is your name?";
  };
  ...
  // Synchronize and translate current message when language changes
  useEffect(() => {
    setMessages((prev) => {
      ...
      // If the latest message is an AI question, dynamically translate it!
      if (lastMsg && lastMsg.sender === 'ai') {
        const qId = lastMsg.questionId || (prev.length === 1 ? 'ask_name' : null);
        if (qId) {
          const translated = getTranslatedQuestion(qId, language, sessionState);
          if (translated) {
            const updated = [...prev];
            updated[lastIdx] = {
              ...lastMsg,
  ...
  const speakVoice = (text, lang = language) => {
    ...
    try {
      ...
      activeUtteranceRef.current = utterance; // Keep active reference to prevent Chromium GC
    ...
    if (SpeechRecognition) {
      ...
      recognition.onresult = (event) => {
        ...
        if (final.trim()) {
          setIsListening(false);
          setLiveTranscript('');
          // Unified Answer Handler: speech-to-text calls exact same function as quick reply buttons
  ...
  const handleUserAnswer = async (answer) => {
    ...
    // Normalizing raw value/label from either button object or speech string
    let textToSend = '';
    ...
    // Stop active mic and speech immediately
    if (isListening && recognitionRef.current) {
      try { recognitionRef.current.stop(); } catch (e) {}
      setIsListening(false);
    }
  ...
  const handleSend = async (textToSend = inputText) => {
    const raw = typeof textToSend === 'object' && textToSend !== null ? (textToSend.value || textToSend.label || '') : textToSend;
    ...
    try {
      ...
      const replyText = response.data?.reply || response.data?.message || "I am analyzing your business profile...";
      const speakTextToUse = response.data?.speak_text || replyText;
      const quickRepliesToUse = response.data?.quickReplies || [];
      ...
      const aiMsg = {
        ...
        quickReplies: quickRepliesToUse,
        business: response.data?.business,
        step: response.data?.step,
        waitingForAnswer: response.data?.waitingForAnswer,
        questionId: response.data?.questionId || null,
  ...
  const currentQuickReplies = latestAiMessage?.quickReplies || [];
    ...
    <div className="bg-white rounded-3xl p-5 sm:p-7 shadow-xl border border-emerald-900/10 flex flex-col justify-between">
          ...
          >
            ...
            `}>
              ...
              {msg.quickReplies && msg.quickReplies.length > 0 && idx === messages.length - 1 && (
                <div className="mt-3 pt-2.5 border-t border-stone-200/70 flex flex-wrap gap-1.5">
                  {msg.quickReplies.map((replyOpt, rIdx) => {
                    const label = typeof replyOpt === 'object' ? replyOpt.label : replyOpt;
                    const val = typeof replyOpt === 'object' ? (replyOpt.value || replyOpt.label) : replyOpt;
                    return (
                      <button
                        key={rIdx}
                        onClick={() => handleUserAnswer(replyOpt)}
                        disabled={isLoading}
      ...
      <div className="flex flex-col items-center justify-center my-3 text-center">
        ...
        <div className="mt-3.5 space-y-1">
          ...
          {/* Contextual Quick Reply Buttons below Microphone (always accessible & resilient to mic failures) */}
          {currentQuickReplies && currentQuickReplies.length > 0 && (
            <div className="mt-4 pt-3 border-t border-stone-200/70 w-full max-w-xl mx-auto text-center animate-in fade-in duration-200">
              <p className="text-xs font-bold text-stone-600 mb-2.5">
                {language === 'hi' ? 'त्वरित विकल्प (टैप करें):' : (language === 'te' ? 'ఎంపికలు (నొక్కండి):' : 'Quick Options (Tap to answer):')}
              </p>
              <div className="flex flex-wrap justify-center gap-2">
                {currentQuickReplies.map((option, optIdx) => {
                  const label = typeof option === 'object' ? option.label : option;
                  const val = typeof option === 'object' ? (option.value || option.label) : option;
                  return (
                    <button
                      key={optIdx}
                      onClick={() => handleUserAnswer(option)}
                      disabled={isLoading}
                      className="px-3.5 py-2 rounded-xl bg-white hover:bg-[#0F3D2E] text-[#0F3D2E] hover:text-amber-300 font-bold text-xs sm:text-sm border-2 border-emerald-600/30 hover:border-[#0F3D2E] transition-all shadow-xs hover:shadow-md transform active:scale-95 cursor-pointer"
                    >
                      {label}
                    </button>
                  );
      ...
      <div className="mt-4 pt-3.5 border-t border-stone-100">
        ...
        <div className="flex flex-wrap gap-2">
          {suggestedPrompts.map((prompt, i) => (
            <button
              key={i}
              onClick={() => handleUserAnswer(prompt)}
